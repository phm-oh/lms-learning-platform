...
app.get("/", (req, res) => {
  res.status(200).json({
    ...
    features: {
      ...
      newsSystem: "✅ News management system",
    },
    endpoints: {
      api: "/api",
      health: "/api/health",
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        formattedTotalSize:
          require("./utils/fileHelper").formatFileSize(totalSize),
        categories: usage,
        limits: {
          maxFileSize: process.env.MAX_FILE_SIZE || "100MB",
...
const setupCleanupTasks = () => {
  ...
  cron.schedule("0 2 * * *", async () => {
    try {
      ...
      console.log(
        `✅ Cleanup completed: ${result.deletedCount || 0} files removed`
      );
    } catch (error) {
...
if (process.env.NODE_ENV === "development") {
  ...
  console.log("\n📁 Upload System Info:");
  console.log(
    `   Storage Type: ${storageManager.getConfig().active?.type || "local"}`
  );
